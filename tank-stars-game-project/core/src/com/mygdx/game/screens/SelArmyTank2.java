...
public class SelArmyTank2 implements Screen {
    private Texture armyTankWhite;
    private MyGame game;

    public SelArmyTank2(MyGame game) {
        this.setGame(game);
    }

    @Override
    public void show() {
        setArmyTankWhite(new Texture(Gdx.files.internal("TankSelectionPics/p22.png")));
    }
    ...
    public void render(float delta) {
        ...
        if (Gdx.input.getX() > 600 && Gdx.input.getX() < 880 && Gdx.input.getY() > 400 && Gdx.input.getY() < 670) {
            if (Gdx.input.justTouched()) {
                dispose();
                game.setScreen(new SelArmyTank2(game));      // HERE ARMY TANK SELECTION CLASS CALLED
            }
        }
